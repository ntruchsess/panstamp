...
sub new(;$) {
  ...
  if (defined $strPacket) {
    ...
    die "Incorrect packet format for incoming data. Lack of (RSSI,LQI)." unless $strPacket =~ /^\([0-9a-f]{4}\)/;
    die "Incorrect packet format. Amount of characters should not be odd." if (length $strPacket) % 2 > 0;
