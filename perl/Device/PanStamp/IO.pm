...
panStamp_DoInit($)
{
  ...
  if( defined( $hash->{NETID} ) ) {
    panStamp_SimpleWrite($hash, "ATSW=$hash->{NETID}" );
  ...
  panStamp_SimpleWrite($hash, "ATDA=01" );
...
panStamp_Write($$$)
{
  ...
  my $bstring = $addr.sprintf( "%02X", $hash->{".address"}).$msg;
...
<ul>
  ...
  <b>Define</b>
  <ul>
    <code>define &lt;name&gt; panStamp &lt;device&gt; &lt;NETID&gt;</code> <br>
    <br>
    USB-connected devices:<br><ul>
      &lt;device&gt; specifies the serial port to communicate with the panStamp or
    ...
    <br>
    The NETID is a 4 digit hex number to identify the panStamp network.
  </ul>
  <br>
