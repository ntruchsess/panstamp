...
SWAP_loadDevices()
{
  ...
  my $file_name = "$attr{global}{modpath}/FHEM/SWAP/devices.xml";
  ...
  foreach my $developer (@{$developers->{developer}}){
    ...
    foreach my $device (@{$developer->{dev}}){
      ...
      #readDeviceXaML( $_products->{$productcode}, "$attr{global}{modpath}/FHEM/SWAP/$_developer->{name}/$name.xml" );
    }
  }
  ...
  if( my $sl = $modules{$hash->{TYPE}}{SWAP_SetList} ) {
    ...
    if( $cmd ne '?' ) {
      if( my @a = grep( $_ =~ /^$cmd($|:)/, keys %{$sl} ) ) {
        return "set $cmd requires $sl->{$a[0]} parameter" if( $sl->{$a[0]} != -99 && $sl->{$a[0]} != $cnt-1 );
...
SWAP_Get($@)
{
  ...
  if( my $gl = $modules{$hash->{TYPE}}{SWAP_GetList} ) {
    ...
    if(defined($gl->{$cmd}) ) {
      return "get $cmd requires $gl->{$cmd} parameter" if( $gl->{$cmd} != int(@args) );
...
SWAP_readDeviceXML($$)
{
  ...
  if( defined($developer->{name}) && defined($product->{name}) ) {
    readDeviceXML( $products->{$productcode}, "$attr{global}{modpath}/FHEM/SWAP/$developer->{name}/$product->{name}.xml" );
  } else {
...
SWAP_updateReadings($$$)
{
  ...
  if( $hash->{reg} && hex($hash->{reg}) != $reg ) {
    ...
    readingsBulkUpdate($hash, "state", $data) if( defined($attr{$name}{ProductCode}) && $attr{$name}{ProductCode} eq '0000002200000003' && $reg == 0x0B );
...
<ul>
  ...
  <tr><td>
  The SWAP protocoll is used by panStamps <a href="http://www.panstamp.com">panstamp.com</a>.
  ...
  <ul>
    ...
    <li>regSet &lt;reg&gt;.&lt;eb&gt &lt;data&gt;<br>
        write &lt;data&gt; to endpoint &lt;eb&gt of register &lt;reg&gt;. will not work if no reading for register &lt;reg&gt; is available as all nibbles that are not part of endpoint &lt;eb&gt will be filled from this reading.
        </li><br>
  ...
  <ul>
    <li>ProductCode<br>
        ...
        hast to be set manualy for devices that in sleep mode during definition.
        </li><br>
  </ul>
  <br>
</ul>
