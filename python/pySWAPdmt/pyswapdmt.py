    ...
    try:
        ...
        if options.config == True:
            ...
            if devAddress is None:
                ...
                print "Enter device in SYNC mode..."
            ...
            if options.regId is not None:
                ...
                if options.value is not None:
                    # Change register
                    if self.manager.setMoteRegister(mote, options.regId, options.value):
                        print "Register modified successfully"
                    else:
                        print "Unable to modify remote register"
                else:
                    # Query register
                    val = self.manager.queryMoteRegister(mote, options.regId)
                    if val is not None:
            ...
            if setCustomReg == True:
                for reg in listCfgRegs:
                    if mote.cmdRegisterWack(reg.id, reg.value) == False:
                        print "Unable to set register " + str(reg.id)
                        break
